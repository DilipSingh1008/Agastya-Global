...
const ProfilePage = () => {
  ...
  const profileData = Array.isArray(raw) ? raw[0] : raw;
  ...
  const profileForm = useFormik({
    ...
    enableReinitialize: true, // re-populates form when profileData loads
    ...
    onSubmit: async (values) => {
      try {
        ...
        await updateItem({ url: "admin/profile", data: formData });
  ...
  // ── Sync photo preview when profile data loads ──
  useEffect(() => {
    if (profileData?.photo) setProfilePhotoPreview(profileData.photo);
  }, [profileData?.photo]);
