...
const AddCmsForm = () => {
  ...
  // ── Populate form & Quill when edit data is ready ──
  useEffect(() => {
    if (!isEdit || !cmsRes || !quill) return;

    const page = cmsRes.data || cmsRes;
    ...
    quill.root.innerHTML = page.mainDescription || "";
  }, [isEdit, cmsRes, quill]);

  // ── Redirect if no permission ──
  useEffect(() => {
    if (isLoading) return;
    if (isEdit && !cmsPermission?.edit) navigate("/dashboard/cms");
    if (!isEdit && !cmsPermission?.add) navigate("/dashboard/cms");
  }, [cmsPermission, isLoading]);
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (isEdit) {
        await updateItem({ url: `cms/update/${id}`, data });
        alert("CMS page updated successfully!");
      } else {
        await createItem({ url: "cms/create", data });
