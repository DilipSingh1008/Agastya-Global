...
const NewsForm = () => {
  ...
  // ── RTK Query: fetch news item for edit ───────────────────────────────────
  const { data: newsItem, isLoading } = useGetItemsQuery(`news/${id}`, {
    skip: !isEdit,
  });
  ...
  // ── Sync form values + quill when data loads ──────────────────────────────
  useEffect(() => {
    if (!newsItem) return;
    setInitialValues({
      title: newsItem.title || "",
      shortDescription: newsItem.shortDescription || "",
      mainDescription: newsItem.mainDescription || "",
      images: [],
      existingImages: newsItem.images || [],
    });
    if (quill) quill.root.innerHTML = newsItem.mainDescription || "";
  }, [newsItem, quill]);
          ...
          onSubmit={async (values, { setSubmitting }) => {
            try {
              ...
              if (isEdit) {
                ...
                await createItem({ url: "news", data }).unwrap();
              }
