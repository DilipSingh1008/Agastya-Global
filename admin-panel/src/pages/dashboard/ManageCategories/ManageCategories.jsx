...
const ManageCategories = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (formData.id) {
        await updateItem({ url: `categories/${formData.id}`, data: payload });
      } else {
        await createItem({ url: "categories", data: payload });
      }
      closeModal();
    } catch (err) {
          ...
          <div className="flex items-center justify-between mb-4 gap-2">
            ...
            {categoryPermission?.add && (
              <button
                ...
                className="cursor-pointer flex items-center gap-1.5 px-3 py-1.5 bg-(--primary) text-white rounded-lg text-xs font-semibold hover:opacity-90 transition-all"
              >
                                ...
                                <div className="relative group">
                                  <button
                                    onClick={() => {
                                      setFormData({
                                        name: cat.name,
                                        icon: `${import.meta.env.VITE_BASE_URL}${cat.icon}`,
                                        id: cat._id,
                                      });
                                      setIsModalOpen(true);
                                    }}
