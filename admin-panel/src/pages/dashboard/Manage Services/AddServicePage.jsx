...
const AddEditServicePage = () => {
  ...
  const loading = catsLoading || (isEdit && serviceLoading);
  ...
  useEffect(() => {
    if (!isEdit || !serviceRes || !quill || categories.length === 0) return;
    ...
    if (s.category?._id) {
      const selectedCat = categories.find((cat) => cat._id === s.category._id);
      setSubCategories(selectedCat?.subCategories || []);
    }
  }, [isEdit, serviceRes, quill, categories]);

  // ── Redirect if no permission ──
  useEffect(() => {
    if (!loading) {
      if (isEdit && !servicePermission?.edit) navigate("/dashboard/service");
      if (!isEdit && !servicePermission?.add) navigate("/dashboard/service");
    }
  }, [servicePermission, loading]);
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (isEdit) {
        await updateItem({ url: `services/${id}`, data: formData });
        alert("Service updated successfully!");
      } else {
        await createItem({ url: "services", data: formData });
