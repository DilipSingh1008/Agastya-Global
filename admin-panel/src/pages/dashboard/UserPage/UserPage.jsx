...
const UserPage = () => {
  ...
  // ── RTK Query: fetch roles for dropdown ──
  const { data: rolesRes } = useGetItemsQuery("role?limit=100");
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      await updateItem({ url: `user/${id}`, data: { status: !currentStatus } });
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (editingUser) {
        await updateItem({ url: `user/${editingUser._id}`, data: formData });
      } else {
        await createItem({ url: "user", data: formData });
      }
      closeModal();
    } catch (err) {
      console.error(err);
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                >
                  <tr>
                    ...
                    {(usersPermission?.edit || usersPermission?.delete) && (
