...
const BlogForm = () => {
  ...
  // ── RTK Query: fetch blog for edit ────────────────────────────────────────
  const { data: blogData, isLoading } = useGetItemsQuery(`blogs/${id}`, {
    skip: !isEdit,
  });
  ...
  // ── Sync form values + quill when blog data loads ─────────────────────────
  useEffect(() => {
    if (!blogData) return;
    setInitialValues({
      title: blogData.title || "",
      shortDescription: blogData.shortDescription || "",
      mainDescription: blogData.mainDescription || "",
      categoryId: blogData.categoryId?._id || "",
      images: [],
      existingImages: blogData.images || [],
    });
    if (quill) quill.root.innerHTML = blogData.mainDescription || "";
  }, [blogData, quill]);
          ...
          onSubmit={async (values, { setSubmitting }) => {
            try {
              ...
              if (isEdit) {
                ...
                await createItem({ url: "blogs", data }).unwrap();
              }
